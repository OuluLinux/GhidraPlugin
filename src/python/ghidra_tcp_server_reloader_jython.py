    ...
    def restart_handler(self, parameters):
        ...
        if server:
            try:
                ...
                print("Error during restart: " + str(e))
...
def start_server(port=9000):
    ...
    # Try to find an available port by creating a temporary socket first
    # This is an additional safety check before creating the server instance
    test_socket = None
    try:
        test_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        test_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        ...
        print("Port {} is available, proceeding with server startup...".format(port))
    except OSError as e:
        print("Port {} is currently in use: {}".format(port, str(e)))
        print("Will try to find an available port automatically...")
        # We'll let the server handle port selection in the find_available_port method
        pass
    finally:
        if test_socket:
            try:
    ...
    try:
        ...
        import time
        time.sleep(0.2)  # Increased delay to ensure server has time to bind
...
def reload_server(port=9000):
    ...
    if server:
        ...
        stop_server()
        # Longer delay to ensure the old server is completely stopped
        # and the OS has released the port
        import time
        time.sleep(1.0)
...
def quick_reload(port=9000):
    ...
    # Instead of re-executing the whole script, just start a fresh server
    start_server(port)
