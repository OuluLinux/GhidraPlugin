"""
Enhanced Ghidra TCP Server with External Reload Capability

This script provides the same TCP server functionality as before but adds the ability
to reload and restart the server from within Ghidra without having to manually type
the full reload command each time. Additionally, adds a remote execution command for
...
class GhidraTCPClientHandler:
    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                if command == "QUIT":
                    break
                # If client sends RESTART, trigger reload
    ...
    def handle_restart(self):
        """Handle server restart command from client"""
        global server
        print("Client requested server restart...")
        if server:
            try:
                ...
                if server.socket:
                    server.socket.close()
                # Set server to None to indicate it's stopped
                server = None
                print("Server marked for restart. Please use reload_server() in Ghidra console.")
            except Exception as e:
                print("Error during restart: {}".format(str(e)))
    ...
    def handle_command(self, command, parameters):
        ...
        command_handlers = {
            ...
            "RESTART": self.restart_handler,
            "RELOAD": self.reload_handler,
        }
    ...
    def restart_handler(self, parameters):
        """Handle RESTART command - triggers server restart"""
        # This is handled in handle_restart, but we provide a response
        return "RESTARTING: Server will restart after connection closes"

    def reload_handler(self, parameters):
        """Handle RELOAD command - reloads and restarts the server"""
        # This function will trigger the global reload_server function
        global server
        print("Client requested server reload...")
        
        # Set a flag on the server instance to indicate reload is requested
        if hasattr(self, 'current_program'):
            # Try to trigger the reload function which should be available in the global scope
            try:
                # In the Ghidra context, we need to make sure the global function is available
                # We can't directly call it here, but we can set a flag to indicate reload is requested
                if 'reload_server' in globals():
                    # Schedule the reload after this connection closes
                    # We'll use a simple flag in the global server instance
                    global reload_requested
                    reload_requested = True
                    return "RELOADING: Server will reload after this connection closes"
                else:
                    return "ERROR: reload_server function not available in Ghidra context"
            except Exception as e:
                return "ERROR: Failed to initiate reload - " + str(e)
        else:
            return "ERROR: Cannot reload, not running in Ghidra context"
...
Available commands:
  ...
  RELOAD                            - Reload and restart the server
  HELP                              - Show this help
...
# Global server instance and reload flag
server = None
...
def reload_server(port=9000):
    """Reload the server by stopping and starting it again"""
    global server, reload_requested
    if server:
        print("Reloading server...")
        stop_server()
        # Small delay to ensure the old server is properly stopped
        import time
        time.sleep(0.2)
...
def quick_reload(port=9000):
    """Quick reload that simulates the original command more reliably"""
    import os
    script_path = os.path.abspath(__file__)

    print("Quick reloading server from: " + script_path)
    ...
    # Instead of exec, we'll just start a fresh server after stopping
    # This approach is more reliable than re-executing the whole script
    start_server(port)

    print("Server quick reloaded successfully!")


def check_reload_requested():
    """Check if reload has been requested from a client and execute it if so"""
    global reload_requested
    if reload_requested:
        print("Executing reload as requested by client...")
        reload_server()
        return True
    return False
...
def auto_reload_checker():
    """Function to run in Ghidra console to periodically check for reload requests"""
    global reload_requested
    import time

    print("Starting auto-reload checker... Press Ctrl+C to stop")
    try:
        while True:
            if reload_requested:
                print("\nReload request detected from client!")
                reload_server()
            time.sleep(1)  # Check every second
    except KeyboardInterrupt:
        print("\nAuto-reload checker stopped")
